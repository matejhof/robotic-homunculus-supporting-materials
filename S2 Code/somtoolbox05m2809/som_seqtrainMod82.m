function [sMap, sTrain] = som_seqtrainMod82(sMap, D, MRF, varargin)
%this function is modified som_seqtrain
%author of modifications of the function som_seqtrain from SOM toolbox (http://www.cis.hut.fi/projects/somtoolbox/)- Zdenek Straka

%for more info see article "Robotic homunculus: Learning of artificial skin
%representation in a humanoid robot motivated by primary
%somatosensory cortex" (2015) by Hoffmann, Straka, Farkas, Vavrecka, Metta 


%modified function implements MRF-SOM (Kohonen's SOM that use dot product for determine a winner neuron with added possibility of setting a maximum range of receptive field(MRF) for each neuron) 
%use for determine winner neuron max dot product of winner neuron and input vector
%MRF matrix have a same size as the sMap codebook
%values in MRF: 0-not connected, value 1-connected
%some functionality of original som_seqtrain was not tested in the current version so may be nonfunctional
%most of original comments (from som_seqtrain) were preserved because both version are
%different only in few lines of code (use compare function)
...
for t = 1:trainlen, 
  ...
  %Dx = M(:,known) - x(mu_x_1,known);     %original som_seqtrain code-each map unit minus the vector
  Dx = - x(mu_x_1,known);    
  Mpom=M(:,known).*MRF(:,known);
  pom=Mpom*x(1,known)'; % dot product with each neuron
  [qerr bmu] = max(pom); %qerr doesnt have original meaning

  %[qerr bmu] = min((Dx.^2)*mask(known)); %original som_seqtrain code-minimum distance(^2) and the BMU
  ...
  M=M.*MRF; %set a zeros in the M where in MRF are a zeros
